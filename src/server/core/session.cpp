...
void ClientSession::processingThread()
{
   ...
   while (true)
   {
      ...
      debugPrintf(3, _T("Received message %s"), NXCPMessageCodeName(m_wCurrentCmd, szBuffer));
      if (!(m_dwFlags & CSF_AUTHENTICATED) &&
      ...
      case CMD_ZMQ_GET_DATA_SUBSCRIPTIONS:
         ...
         break;
#endif
      case CMD_GET_GOOGLE_ACCOUNT:
         getGoogleAccount(pMsg);
         break;
      case CMD_GET_HOME_CHANNEL:
         getHomeChannels(pMsg);
         break;
      case CMD_GET_MONITOR_CHANNEL:
         getMonitorChannels(pMsg);
         break;
      case CMD_GET_MAPPING_CHANNEL:
         getMappingConfig(pMsg);
         break;
      case CMD_CREATE_GOOGLE_ACCOUNT:
         createGoogleAccount(pMsg);
         break;
      case CMD_CREATE_HOME_CHANNEL:
         createHomeChannel(pMsg);
         break;
      case CMD_CREATE_MONITOR_CHANNEL:
         createMonitorChannel(pMsg);
         break;
      case CMD_CREATE_MAPPING_CHANNEL:
         createMappingChannel(pMsg);
         break;
      case CMD_MOD_GOOGLE_ACCOUNT:
         modifyGoogleAccount(pMsg);
         break;
      case CMD_MOD_HOME_CHANNEL:
         modifyHomeChannel(pMsg);
         break;
      case CMD_MOD_MONITOR_CHANNEL:
         modifyMonitorChannel(pMsg);
         break;
      case CMD_MOD_MAPPING_CHANNEL:
         modifyMappingChannel(pMsg);
         break;
      case CMD_DEL_GOOGLE_ACCOUNT:
         deleteGoogleAccount(pMsg);
         break;
      case CMD_DEL_HOME_CHANNEL:
...
void ClientSession::getGoogleAccount(NXCPMessage *request)
{
   ...
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   DB_STATEMENT hStmt = DBPrepare(hdb, _T("SELECT * FROM google_account"));
   if (hStmt != NULL)
   {
      hResult = DBSelectPrepared(hStmt);
      if (hResult != NULL)
      {
         ...
         for (i = 0, dwId = VID_VARLIST_BASE; i < dwNumRecords; i++, dwId += 10)
         {
            msg.setField(dwId , DBGetField(hResult, i, 0, NULL, 0));       //user_name
            msg.setField(dwId + 1, DBGetField(hResult, i, 1, NULL, 0));    //api
            msg.setField(dwId + 2, DBGetField(hResult, i, 2, NULL, 0));    //client_secret
            msg.setField(dwId + 3, DBGetField(hResult, i, 3, NULL, 0));    //client_id
            msg.setField(dwId + 4, DBGetFieldInt64(hResult, i, 4));        //account_type
            msg.setField(dwId + 5, DBGetField(hResult, i, 5, NULL, 0));    //app_name
         }
         DBFreeResult(hResult);
      }
      DBFreeStatement(hStmt);
...
void ClientSession::getHomeChannels(NXCPMessage *request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_RESULT hResult;
   ...
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   DB_STATEMENT hStmt = DBPrepare(hdb, _T("SELECT channel_id, channel_name, google_account_user_name, video_number, " )
                                  _T(" view_number, subscriber, date_created, status, note FROM home_channel"));
   if (hStmt != NULL)
   {
      hResult = DBSelectPrepared(hStmt);
      if (hResult != NULL)
      {
         ...
         for (i = 0, dwId = VID_VARLIST_BASE; i < dwNumRecords; i++, dwId += 10)
         {
            ...
            msg.setField(dwId + 2, DBGetField(hResult, i, 2, NULL, 0));    //google_account_id
            msg.setField(dwId + 3, DBGetFieldInt64(hResult, i, 3));        //video number
            msg.setField(dwId + 4, DBGetFieldInt64(hResult, i, 4));        //view number
            msg.setField(dwId + 5, DBGetFieldInt64(hResult, i, 5));        //subscriber
            msg.setField(dwId + 6, DBGetFieldInt64(hResult, i, 6));        //date created
            msg.setField(dwId + 7, DBGetFieldInt64(hResult, i, 7));        //status
            msg.setField(dwId + 8, DBGetField(hResult, i, 8, NULL, 0));    //note
         }
         DBFreeResult(hResult);
      }
      DBFreeStatement(hStmt);
...
void ClientSession::getMonitorChannels(NXCPMessage *request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_RESULT hResult;
   ...
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   DB_STATEMENT hStmt = DBPrepare(hdb, _T("SELECT channel_id, channel_name , video_number, view_number, ")
                                  _T(" subscriber, date_created, status, note FROM monitor_channel"));
   if (hStmt != NULL)
   {
      hResult = DBSelectPrepared(hStmt);
      if (hResult != NULL)
      {
         ...
         for (i = 0, dwId = VID_VARLIST_BASE; i < dwNumRecords; i++, dwId += 10)
         {
            msg.setField(dwId, DBGetField(hResult, i, 0, NULL, 0));        //channel_id
            msg.setField(dwId + 1, DBGetField(hResult, i, 1, NULL, 0));    //channel_name
            msg.setField(dwId + 2, DBGetFieldInt64(hResult, i, 2));        //video number
            msg.setField(dwId + 3, DBGetFieldInt64(hResult, i, 3));        //view number
            msg.setField(dwId + 4, DBGetFieldInt64(hResult, i, 4));        //subscriber
            msg.setField(dwId + 5, DBGetFieldInt64(hResult, i, 5));        //date created
            msg.setField(dwId + 6, DBGetFieldInt64(hResult, i, 6));        //status
            msg.setField(dwId + 7, DBGetField(hResult, i, 7, NULL, 0));    //note
         }
         DBFreeResult(hResult);
      }
      DBFreeStatement(hStmt);
...
void ClientSession::getMappingConfig(NXCPMessage *request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_RESULT hResult;
...
void ClientSession::createGoogleAccount(NXCPMessage *request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
...
void ClientSession::createHomeChannel(NXCPMessage *request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   ...
   if (hdb != NULL)
   {
      ...
      TCHAR* googleUser = request->getFieldAsString(VID_HOME_CHANNEL_ACCOUNT_ID);
      hStmt = DBPrepare(hdb, _T("INSERT INTO home_channel (channel_id, google_account_user_name) VALUES (?,?)"));
      DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, (const TCHAR *)cId, DB_BIND_TRANSIENT);
      DBBind(hStmt, 2, DB_SQLTYPE_VARCHAR, (const TCHAR*)googleUser, DB_BIND_TRANSIENT);
      bool success = DBExecute(hStmt);
      if (success == true)
      {
...
void ClientSession::createMonitorChannel(NXCPMessage *request)
{
   ...
   if (hdb != NULL)
   {
      ...
      hStmt = DBPrepare(hdb, _T("INSERT INTO monitor_channel (channel_id) VALUES (?)"));
      DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, (const TCHAR *)cId, DB_BIND_TRANSIENT);
      bool success = DBExecute(hStmt);
      if (success == true)
      {
...
void ClientSession::createMappingChannel(NXCPMessage *request)
{
   ...
   {
      // Prepare and execute INSERT or UPDATE query
      INT32 mappingType = request->getFieldAsInt32(VID_MAPPING_CHANNEL_TYPE);
      INT32 statusSync = request->getFieldAsInt32(VID_MAPPING_CHANNEL_STATUS_SYNC);
      INT32 timeSync = request->getFieldAsInt32(VID_MAPPING_CHANNEL_TIME_SYNC);
      TCHAR* cHomeId = request->getFieldAsString(VID_MAPPING_CHANNEL_HOME_ID);
      TCHAR* monitorContent = request->getFieldAsString(VID_MAPPING_CHANNEL_MONITOR_CONTENT);
      TCHAR* downloadId = request->getFieldAsString(VID_MAPPING_CHANNEL_DOWNLOAD_CLUSTER_ID);
      TCHAR* renderId = request->getFieldAsString(VID_MAPPING_CHANNEL_RENDER_CLUSTER_ID);
      TCHAR* uploadId = request->getFieldAsString(VID_MAPPING_CHANNEL_UPLOAD_CLUSTER_ID);
      INT32 lastSyncTime = request->getFieldAsInt32(VID_MAPPING_CHANNEL_LAST_SYNC_TIME);

      hStmt = DBPrepare(hdb, _T("INSERT INTO mapping_list (mapping_type, status_sync, time_interval_sync, ")
                        _T(" home_channel_id, monitor_content, download_cluster, render_cluster, upload_cluster, ")
                        _T(" last_sync_time) VALUES (?,?,?,?,?,?,?,?,?)"));
      DBBind(hStmt, 1, DB_SQLTYPE_INTEGER, mappingType);
      DBBind(hStmt, 2, DB_SQLTYPE_INTEGER, statusSync);
      DBBind(hStmt, 3, DB_SQLTYPE_INTEGER, timeSync);
      DBBind(hStmt, 4, DB_SQLTYPE_VARCHAR, (const TCHAR *)cHomeId, DB_BIND_TRANSIENT);
      DBBind(hStmt, 5, DB_SQLTYPE_VARCHAR, (const TCHAR *)monitorContent, DB_BIND_TRANSIENT);
      DBBind(hStmt, 6, DB_SQLTYPE_VARCHAR, (const TCHAR *)downloadId, DB_BIND_TRANSIENT);
      DBBind(hStmt, 7, DB_SQLTYPE_VARCHAR, (const TCHAR *)renderId, DB_BIND_TRANSIENT);
      DBBind(hStmt, 8, DB_SQLTYPE_VARCHAR, (const TCHAR *)uploadId, DB_BIND_TRANSIENT);
      DBBind(hStmt, 9, DB_SQLTYPE_VARCHAR, lastSyncTime);
      bool success = DBExecute(hStmt);
      if (success == true)
      {
         msg.setField(VID_RCC, RCC_SUCCESS);
         ...
         if (statusSync == 1) //enable timer
         {
            ...
            NetxmsCorbaClient* downloadClient = new NetxmsCorbaClient((const TCHAR*)downloadId);
            if (downloadClient->initSuccess)
            {
               try {
                  downloadClient->netxmsClientRef->createDownloadTimer(mappingId, timeSync);
               }
               catch (CORBA::TRANSIENT&) {
                  debugPrintf(1, _T("Caught system exception TRANSIENT -- unable to contact the server"));
               }
            ...
            NetxmsCorbaClient* uploadClient = new NetxmsCorbaClient((const TCHAR*)uploadId);
            if (uploadClient->initSuccess)
            {
               try
               {
                  uploadClient->netxmsClientRef->createUploadTimer(::CORBA::wstring_dup(cHomeId));
               }
               catch (CORBA::TRANSIENT&) {
                  debugPrintf(1, _T("Caught system exception TRANSIENT -- unable to contact the server"));
               }
...
void ClientSession::modifyGoogleAccount(NXCPMessage * request)
{
   ...
   if (hdb != NULL)
   {
      ...
      hStmt = DBPrepare(hdb, _T("UPDATE google_account SET api = ?, client_secret = ?, ")
                        _T(" client_id = ?, account_type = ?, app_name = ? WHERE user_name = ?"));

      DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, (const TCHAR *)api, DB_BIND_TRANSIENT);
      DBBind(hStmt, 2, DB_SQLTYPE_VARCHAR, (const TCHAR *)clientSecret, DB_BIND_TRANSIENT);
      DBBind(hStmt, 3, DB_SQLTYPE_VARCHAR, (const TCHAR *)clientId, DB_BIND_TRANSIENT);
...
void ClientSession::modifyHomeChannel(NXCPMessage * request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   ...
   if (hdb != NULL)
   {
      ...
      hStmt = DBPrepare(hdb, _T("UPDATE home_channel SET ")
                        _T(" google_account_user_name = ? WHERE channel_id = ?"));
      DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, (const TCHAR*) googleUser, DB_BIND_TRANSIENT);
...
void ClientSession::modifyMonitorChannel(NXCPMessage * request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   /*
   NXCPMessage msg;
...
void ClientSession::modifyMappingChannel(NXCPMessage * request)
{
   ...
   if (hdb != NULL)
   {
      ...
      hStmt = DBPrepare(hdb, _T("UPDATE mapping_list SET mapping_type = ?, status_sync = ?, time_interval_sync = ?, ")
                        _T(" home_channel_id = ?, monitor_content = ?, download_cluster = ?, render_cluster = ?,")
                        _T(" upload_cluster = ? WHERE id = ?"));
      DBBind(hStmt, 1, DB_SQLTYPE_INTEGER, mappingType);
      DBBind(hStmt, 2, DB_SQLTYPE_INTEGER, statusSync);
      DBBind(hStmt, 3, DB_SQLTYPE_INTEGER, timeSync);
      DBBind(hStmt, 4, DB_SQLTYPE_VARCHAR, (const TCHAR *)cHomeId, DB_BIND_TRANSIENT);
      ...
      DBBind(hStmt, 7, DB_SQLTYPE_VARCHAR, (const TCHAR *)renderId, DB_BIND_TRANSIENT);
      DBBind(hStmt, 8, DB_SQLTYPE_VARCHAR, (const TCHAR *)uploadId, DB_BIND_TRANSIENT);
      DBBind(hStmt, 9, DB_SQLTYPE_INTEGER, mappingId);
      ...
      if (success == true)
      {
         ...
         {
            try {
               downloadClient->netxmsClientRef->modifyDownloadTimer((INT32)mappingId, (INT32)timeSync, (INT32)statusSync);
            }
            catch (CORBA::TRANSIENT&) {
               debugPrintf(1, _T("Caught system exception TRANSIENT -- unable to contact the server"));
            }
...
void ClientSession::deleteGoogleAccount(NXCPMessage * request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   ...
   {
      TCHAR* userName = request->getFieldAsString(VID_GOOGLE_USER_NAME);
      hStmt = DBPrepare(hdb, _T("DELETE FROM google_account WHERE user_name = ?"));
      DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, userName, DB_BIND_TRANSIENT);
      bool success = DBExecute(hStmt);
      if (success == true)
      {
         msg.setField(VID_RCC, RCC_SUCCESS);
...
void ClientSession::deleteHomeChannel(NXCPMessage* request)
{
   ...
   if (isDelete)
   {
      if (hdb != NULL)
      {
         hStmt = DBPrepare(hdb, _T("DELETE FROM home_channel WHERE channel_id = ?"));
...
void ClientSession::deleteMappingChannel(NXCPMessage * request)
{
   debugPrintf(6, _T("ClientSession::[%s]"), __FUNCTION__);
   NXCPMessage msg;
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   ...
   if (hdb != NULL)
   {
      ...
      hStmt = DBPrepare(hdb, _T("DELETE FROM mapping_list WHERE id = ?"));
      DBBind(hStmt, 1, DB_SQLTYPE_INTEGER, mappingId);
      bool success = DBExecute(hStmt);
      if (success == true)
      {
         ...
         if (downloadClient->initSuccess)
         {
            ...
            try {
               downloadClient->netxmsClientRef->deleteDowloadTimer(mappingId);
            }
            catch (CORBA::TRANSIENT&) {
               debugPrintf(1, _T("Caught system exception TRANSIENT -- unable to contact the server"));
            }
         ...
         if (uploadClient->initSuccess)
         {
            ...
            try {
               ...
               if (isDelete)
               {
                  uploadClient->netxmsClientRef->deleteUploadTimer(cHomeId);
               }
            }
            catch (CORBA::TRANSIENT&) {
               debugPrintf(1, _T("Caught system exception TRANSIENT -- unable to contact the server"));
...
INT32 ClientSession::getLastestInsertId(TCHAR * tbName)
{
   ...
   if (hStmt != NULL)
   {
      //DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, (const TCHAR *)tbName, DB_BIND_TRANSIENT);
      hResult = DBSelectPrepared(hStmt);
      if (hResult != NULL)
      {
...
AgentConnection * ClientSession::getAgentConnectionByObjectName(TCHAR * objectName)
{
   AgentConnection *agentConn = NULL;
   // Resolve hostname
   InetAddress addr = InetAddress::resolveHostName(objectName);
   debugPrintf(2, _T("ClientSession::[%s] address v4 = %d"), __FUNCTION__, addr.getAddressV4());
   if (addr.isValid())
   {
      ...
      else
      {
         ...
         agentConn = new AgentConnection(addr, AGENT_LISTEN_PORT, AUTH_NONE, _T(""));
         agentConn->setCommandTimeout(g_agentCommandTimeout);
         if (!agentConn->connect(g_pServerKey))
         {
            debugPrintf(2, _T("ClientSession:: Can not create connection to agent ..."));
...
bool ClientSession::checkMappingIsExisted(TCHAR * cHomeId, TCHAR * cMonitorId)
{
   ...
   DB_HANDLE hdb = DBConnectionPoolAcquireConnection();
   DB_STATEMENT hStmt = DBPrepare(hdb, _T("SELECT COUNT(*) FROM channel_mapping WHERE HomeChannelId = ? AND MonitorChannelId = ?"));
   DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, (const TCHAR *)cHomeId, DB_BIND_TRANSIENT);
   ...
   if (hStmt != NULL)
   {
      ...
      if (hResult != NULL)
      {
         int numRecord = DBGetFieldInt64(hResult, 0, 0);
         if (numRecord > 0)
         {
            isExisted = true;
         }
...
void ClientSession::getCluster(NXCPMessage * request)
{
   ...
   hStmt = DBPrepare(hdb, _T("SELECT cluster_id, cluster_name, ip_address, port, ")
   ...
   if (hStmt != NULL)
   {
      ...
      if (hResult != NULL)
      {
         ...
         for (i = 0, dwId = VID_VARLIST_BASE; i < dwNumRecords; i++, dwId += 10)
         {
            msg.setField(dwId + 0, DBGetField(hResult, i, 0, NULL, 0));    // cluster_id
            msg.setField(dwId + 1, DBGetField(hResult, i, 1, NULL, 0));    // cluster_name
            msg.setField(dwId + 2, DBGetField(hResult, i, 2, NULL, 0));    // ip_address
            msg.setField(dwId + 3, DBGetFieldInt64(hResult, i, 3));        // port
            msg.setField(dwId + 4, DBGetField(hResult, i, 4, NULL, 0));    // user_name
            msg.setField(dwId + 5, DBGetField(hResult, i, 5, NULL, 0));    // password
         }
         DBFreeResult(hResult);
      }
      DBFreeStatement(hStmt);
...
void ClientSession::modifyCluster(NXCPMessage * request)
{
   ...
   if (hdb != NULL)
   {
      ...
      TCHAR* password = request->getFieldAsString(VID_SPIDER_CLUSTER_PASSWORD);
      UINT32 clusterType = request->getFieldAsUInt32(VID_SPIDER_CLUSTER_TYPE);
      hStmt = DBPrepare(hdb, _T("UPDATE cluster_info SET cluster_type = ?, cluster_name = ?, ")
                        _T("  ip_address = ?, port = ?, user_name = ?, password = ? WHERE cluster_id = ?"));


      DBBind(hStmt, 1, DB_SQLTYPE_INTEGER, (INT32)clusterType);
      ...
      DBBind(hStmt, 5, DB_SQLTYPE_VARCHAR, (const TCHAR *)userName, DB_BIND_TRANSIENT);
      DBBind(hStmt, 6, DB_SQLTYPE_VARCHAR, (const TCHAR *)password, DB_BIND_TRANSIENT);
      DBBind(hStmt, 7, DB_SQLTYPE_VARCHAR, (const TCHAR *)clusterId, DB_BIND_TRANSIENT);
      bool success = DBExecute(hStmt);
      if (success == true)
      {
         msg.setField(VID_RCC, RCC_SUCCESS);
...
void ClientSession::deleteCluster(NXCPMessage * request)
{
   ...
   if (hdb != NULL)
   {
      TCHAR* cluster_id = request->getFieldAsString(VID_SPIDER_CLUSTER_ID);
      hStmt = DBPrepare(hdb, _T("DELETE FROM cluster_info WHERE cluster_id = ?"));

      DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, (const TCHAR*)cluster_id, DB_BIND_TRANSIENT);
      bool success = DBExecute(hStmt);
      if (success == true)
      {
         msg.setField(VID_RCC, RCC_SUCCESS);
...
void ClientSession::updateChannelInfo(const TCHAR* tbName, ChannelData* cData)
{
   ...
   if (hdb != NULL)
   {
      hStmt = DBPrepare(hdb, _T("UPDATE home_channel SET channel_name = ?, video_number = ?, ")
