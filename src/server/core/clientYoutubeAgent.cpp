...
YoutubeAgent::YoutubeAgent(const TCHAR* contextName): initSuccess(false)
{
	try {
		int agrc = 1;
		char* agrv[] = { ""};
		...
		{
			mYtAgentRef = SpiderCorba::YoutubeAgent::_narrow(obj);
			if (mYtAgentRef != CORBA::Object::_nil())
			{
				initSuccess = true;
			}
		}
...
CORBA::Object_ptr YoutubeAgent::getObjectReference(CORBA::ORB_ptr orb, const TCHAR* contextName)
{
